...
def download_from_google_drive(file_id, destination):
    ...
    try:
        ...
        if response.headers.get('content-type', '').startswith('text/html'):
            print(f"⚠️  Large file detected, handling virus scan warning...")
        ...
        if file_size < 1000 and 'bert_model' in str(destination):
            print(f"⚠️  Warning: BERT model file is very small ({file_size} bytes), might be corrupted")
            return False
...
def download_models():
    ...
    for filename, file_id in GOOGLE_DRIVE_FILES.items():
        if file_id == f"YOUR_{filename.upper().replace('.', '_').replace('/', '_')}_FILE_ID":
            print(f"⚠️  Skipping {filename} - file ID not configured")
            continue
...
def create_model_placeholders():
    ...
    print("⚠️  Created placeholder directories for missing models")
...
def main():
    ...
    if os.environ.get('RENDER'):
        ...
        if model_dir.exists() and any(model_dir.iterdir()):
            ...
            print("⚠️  No local models found, creating placeholders...")
